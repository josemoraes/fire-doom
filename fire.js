...
function createFireDataStructure() {
  ...
  for (let pixelPosition = 0; pixelPosition < numberOfPixels; pixelPosition++) {
    firePixelArray[pixelPosition] = 0;
  }
}
...
function renderFire() {
  ...
  for (let row = 0; row < fireHeight; row++) {
    ...
    for (let column = 0; column < fireWidth; column++) {
      ...
      if (debug) {
        html += `<td><div class="debug-pixel-index">${pixelIndex}</div><span style="color: rgb(${colorString}); font-weight: bold;">${fireIntensity}</span></td>`;
      } else {
